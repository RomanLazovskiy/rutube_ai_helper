...
class AiHelper:
    ...
    def __init__(self, llm, retriever, cross_encoder, top_n_relevant:int=5):
        ...
        llm_template = '''Ты интеллектуальный помощник оператора службы поддержки видеохостинга RUTUBE. Твоя задача максимально точно и вежливо отвечать на запросы пользователя только о платформе. Недобавляй ничего лишнего, используй только предоставленные вопросы и ответы, ничего не выдумывай, не выдавай никакой информации из базы знаний и даже не упоминай о ней, пиши только сам овтет на заданый вопрос. Если среди предоставленных вопросов и ответов нет релевантных, то ответь что не знаешь ответа на предоставленный вопрос. Будь вежливым и дружелюбным. Если ты будешь хорошо показывать результаты, то мы заплатим тебе 200 долларов.
